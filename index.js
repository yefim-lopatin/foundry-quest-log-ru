(() => {
      ...
      for (const [id, markerData] of Object.entries(markers)) {
        ...
        if (page.isOwner && !this._lockPins) {
          ...
          marker.addEventListener("mouseup", (e) => {
            ...
            const markers2 = page.getFlag(MODULE_ID, "markers") ?? {};
            markers2[id].x = xPercent;
            markers2[id].y = yPercent;
            page.setFlag(MODULE_ID, "markers", markers2);
          });
        }
        if (markerData.journal) {
          marker.addEventListener("click", async (e) => {
            ...
            let isQuest = false;
            const isMap = Array.from(mapsJournal.pages).some((page2) => page2.uuid == journal.uuid);
            for (const qJournal of questJournals) {
              qJournal.pages.forEach((page2) => {
                if (page2.uuid == journal.uuid) isQuest = true;
              });
            }
            if (isQuest || isMap) {
  ...
  var TimelineConfig = class extends FormApplication {
    ...
    async _updateObject(event, formData) {
      ...
      for (const page of pages) {
        ...
        updates.push({ _id: page.id, sort: sort2 });
      }
  ...
  var MODULE_ID2 = "foundry-quest-log-ru";
  var FULL_FORMAT = `${MODULE_ID2}/full-journals`;
  var QUESTS_FORMAT = `${MODULE_ID2}/quests`;
  ...
  function validatePage(page) {
    return isObject(page) && typeof page.name === "string" && page.name.trim() && page.type === "text" && isObject(page.text) && typeof page.text.content === "string";
  }
  ...
  function validateLegacyPayload(payload) {
    ...
    for (const quest of payload.quests) {
      ...
      if (quest.description !== void 0 && typeof quest.description !== "string") throw new Error("\u041E\u043F\u0438\u0441\u0430\u043D\u0438\u0435 \u0437\u0430\u043F\u0438\u0441\u0438 \u0434\u043E\u043B\u0436\u043D\u043E \u0431\u044B\u0442\u044C \u0442\u0435\u043A\u0441\u0442\u043E\u043C.");
      if (quest.gmNotes !== void 0 && typeof quest.gmNotes !== "string") throw new Error("\u0417\u0430\u043C\u0435\u0442\u043A\u0438 \u0432\u0435\u0434\u0443\u0449\u0435\u0433\u043E \u0434\u043E\u043B\u0436\u043D\u044B \u0431\u044B\u0442\u044C \u0442\u0435\u043A\u0441\u0442\u043E\u043C.");
      if (quest.objectives !== void 0 && !Array.isArray(quest.objectives)) throw new Error("\u0426\u0435\u043B\u0438 \u0437\u0430\u043F\u0438\u0441\u0438 \u0434\u043E\u043B\u0436\u043D\u044B \u0431\u044B\u0442\u044C \u0441\u043F\u0438\u0441\u043A\u043E\u043C.");
    }
    return payload;
  }
  ...
  function legacyPage(name, content, hidden, quest, suffix = "") {
    return {
      ...
      flags: { [MODULE_ID2]: statusFlags(quest, hidden) }
    };
  }
  function legacyJournal(quest, sort2) {
  ...
  var deepClone = (value) => foundry.utils.deepClone(value);
  var isObject2 = (value) => value && typeof value === "object" && !Array.isArray(value);
  var sameFolder = (journal, folder) => journal.folder === folder || journal.folder === folder?.id || journal.folder?.id === folder?.id;
  ...
  function journalSource(journal, section) {
    return {
      ...
      pages: Array.from(journal.pages).filter((page) => page.type === "text").map(pageSource)
    };
  }
  function exportPayload() {
    const folders = managedFolders();
    const journals = Array.from(game.journal).filter((journal) => isManagedJournal(journal, folders)).map((journal) => journalSource(journal, folders.lore && sameFolder(journal, folders.lore) ? "lore" : "quests")).filter((journal) => journal.pages.length > 0).sort((left, right) => left.section.localeCompare(right.section) || left.sort - right.sort || left.name.localeCompare(right.name, "ru"));
    return {
      format: FULL_FORMAT,
      version: 1,
  ...
  function cleanPage(page) {
    return {
      ...
      ownership: isObject2(page.ownership) ? deepClone(page.ownership) : {},
      title: isObject2(page.title) ? deepClone(page.title) : {},
      sort: Number.isFinite(page.sort) ? page.sort : 0,
      flags: isObject2(page.flags) ? deepClone(page.flags) : {}
    };
  }
  ...
  async function replaceManagedJournals(payload) {
    ...
    try {
      for (const journal of normalized.journals) {
        created.push(await JournalEntry.create({
          ...
          ownership: isObject2(journal.ownership) ? deepClone(journal.ownership) : { default: CONST.DOCUMENT_OWNERSHIP_LEVELS.NONE },
          pages: journal.pages.map(cleanPage)
        }));
      }
    } catch (error) {
      await Promise.all(created.map((journal) => journal.delete().catch(() => void 0)));
      throw error;
    }
...
`], { type: "application/json" });
    ...
    link.href = href;
    link.download = `quest-log-export-${(/* @__PURE__ */ new Date()).toISOString().slice(0, 10)}.json`;
    link.click();
  ...
  function exportManagedJournals() {
  ...
  async function importManagedJournals() {
    if (!game.user.isGM) return ui.notifications.warn(game.i18n.localize(`${MODULE_ID}.importExport.gmOnly`));
  ...
  var SimpleQuest = class _SimpleQuest extends Application {
    ...
    async activateListeners(html) {
      ...
      html.querySelectorAll("#move").forEach((el) => {
        el.addEventListener("change", async (e) => {
          ...
          const moved2 = await newJournal.createEmbeddedDocuments("JournalEntryPage", [page.toObject()]);
          if (!moved2[0]) return;
    ...
    async _onSelectQuest(event, uuid, firstRender = false) {
      ...
      container.querySelectorAll("secret-block").forEach((s) => s.addEventListener("change", (event2) => {
        page.update({ "text.content": event2.target.toggleRevealed(page.text.content) });
      }));
      if (!uuid) {
      ...
      for (const [page, text] of this.textContentIndex) {
        ...
        if (matches2.length) {
          matches2.forEach((m) => {
            ...
            results.push({ page, bestMatch: surroundingText, matchIndex: matches2.indexOf(m) });
          });
        }
  ...
  async function applyTemplate(t, page, app) {
    ...
    if (!hasImage) {
      ...
      await page.update({ "text.content": newContent2 });
      app.render(true);
      return;
    }
